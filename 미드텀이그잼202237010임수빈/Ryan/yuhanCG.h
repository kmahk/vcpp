...
void DrawBonobono(HDC hdc, POINT ptStart, POINT ptEnd, int blink) {
    // 얼굴을 그립니다.
    DrawFace(hdc, ptStart, ptEnd);
    ...
    // 눈의 크기를 정의합니다.
    int eyeWidth = (ptEnd.x - ptStart.x) / 10;
    int eyeHeight = (ptEnd.y - ptStart.y) / 7;

    // blink 매개변수에 따라 눈을 그립니다.
    if (blink % 2 == 0) {
        // 일반 눈을 그립니다.
        DrawEyes(hdc, centerX, centerY, eyeWidth, eyeHeight);
    }
    else {
        // 스페이스바가 눌린 효과로 눈을 그립니다.
        DrawSpaceEyes(hdc, centerX, centerY, eyeWidth, eyeHeight);
    }
}
